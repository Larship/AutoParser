 ...
 */
class scss_server {
	private $sInputFilename = "";
	
	/**
	 * Устанавливает название файла, который требуется отслеживать.
	 * @param string $_filename Название файла, который требуется отслеживать, исключая путь к нему.
	 */
	public function setInputFilename($_filename)
	{
		$this->sInputFilename = $_filename;
	}
	...
	protected function inputName() {
		switch (true) {
			case !empty($this->sInputFilename):
	...
	/**
	 * Компилирует необходимый SCSS-файл и возвращает полный путь к нему.
	 */
	public function serveBackground()
	{
		if($input = $this->findInput())
		{
			$output = $this->cacheName($input);
